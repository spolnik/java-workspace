...
public class ShoppingCartServlet extends HttpServlet {
    ...
    private static final long serialVersionUID = -213223423L;
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (product.isPresent() && quantity >= 0) {
            ShoppingItem shoppingItem = new ShoppingItem(product.get(), quantity);
            updateCart(req, shoppingItem);
        }

        sendProductList(resp);
    }

    private void updateCart(HttpServletRequest req, ShoppingItem shoppingItem) {

        HttpSession session = req.getSession();

        List<ShoppingItem> cart = getShoppingCart(session);
    ...
    private void sendProductDetails(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        Optional<Product> productHolder = getProduct(
                productIdFrom(req)
        );

        if (productHolder.isPresent()) {
            Product product = productHolder.get();
    ...
    private int productIdFrom(HttpServletRequest req) {
        int productId = 0;
        try {
    ...
    private List<ShoppingItem> getShoppingCart(HttpSession session) {

        return (List<ShoppingItem>) session.getAttribute(CART_ATTRIBUTE);
    }
}
